        ...
        // const int N, const int H, const int D )
{
    ...
    if (threadIdx.x < Q)
        shared_inv_freq[threadIdx.x] = fwd / powf(base, (float)threadIdx.x/float(Q));
    __syncthreads();
    ...
    */
    ...
    for (int h = 0; h < H; h++)
    {
        ...
        shared[threadIdx.x] = (float)tokens[b][n][h][threadIdx.x];
        __syncthreads();
        ...
        // write output
        if ((threadIdx.x % (D/2)) < Q)
            tokens[b][n][h][threadIdx.x] = (scalar_t)(u*cos - v*sin);
        else
            tokens[b][n][h][threadIdx.x] = (scalar_t)(v*cos + u*sin);
    }
}
...
void rope_2d_cuda( torch::Tensor tokens, const torch::Tensor pos, const float base, const float fwd ) 
{
    ...
    TORCH_CHECK(tokens.is_contiguous(), "zx/tokens are not contiguous");
    TORCH_CHECK(pos.is_contiguous(), "zx/positions are not contiguous");
    TORCH_CHECK(pos.size(0) == B && pos.size(1) == N && pos.size(2) == 2, "zx/bad pos.shape");
    TORCH_CHECK(D % 4 == 0, "zx/token dim must be multiple of 4");
